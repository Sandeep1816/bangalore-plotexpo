...
export default function BlogGridClient({ posts }: BlogGridClientProps) {
  ...
  const renderCard = (post: any, index: number) => {
    ...
    const cardClasses =   `relative cursor-pointer overflow-hidden rounded-lg h-full group border-2 border-[#00620F] `
    ...
    return (
      <div key={post._id} className={cardClasses} onClick={onClick}>
        ...
        <div className="bg-white p-4 md:p-6 flex flex-col align-center ">
          <p className="text-sm text-[#235B81] font-bold">
  ...
  return (
    <div>
      {/* Header */}
<div className=" mx-auto py-12 px-4 sm:px-6 md:px-10 flex flex-col items-center justify-center">
  <h2 className="text-3xl md:text-4xl font-semibold text-[#00620F] text-center">
    News & Blogs
  </h2>
</div>
